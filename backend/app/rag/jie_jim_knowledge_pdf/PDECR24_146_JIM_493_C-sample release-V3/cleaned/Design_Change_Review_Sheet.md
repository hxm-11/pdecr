...
| If Yes, System impaction evaluation? / 如果是，系统影响评估？（如果是初次样件放行，空白处请说明样件目的） | 催化剂方案可能有变更，封装无影响 |
| Whether need sample to test? Plan? / 是否需要样件进行验证？计划？ |  |
| Whether the catalyst information is confirmed? / 催化剂信息是否确认？（物料号、图纸、SPEC） | 催化剂可能有变更，经系统测试确认 |
...
| If impact customer, whether get confirm from customer side? / 请说明影响和是否得到客户的确认？ | 客户已核类完成边界 |
...
| If Yes, How to evaluate? Plan? / 如果是，请说明影响和如何评估，计划？（例如：仿真分析等） | DV验证过，吊钩和吊钩评估结构强度 |
| Whether need sample to test? Plan? / 是否需要样件进行验证？计划？ | DV马金江通过（发动机耐久、总成热振动） |
...
| 角色 | 姓名 | 签名 | 日期 |
|---|---|---|---|
| Design Team Leader / 设计团队负责人 | 杨广拓 | 杨广拓 |  |
