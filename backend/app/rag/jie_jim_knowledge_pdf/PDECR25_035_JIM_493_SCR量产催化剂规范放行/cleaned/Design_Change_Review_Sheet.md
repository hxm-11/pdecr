...
| Version | 13 |
| Effective date | 2025-04-17th |
| Design Change Request No. | 25_035 |
...
| If Yes, System impaction evaluation? / 如果是，系统影响评估？（如果是初次样件放行，空白处请说明样件目的） | 否，ATSPL验证已完成 |
| Whether need sample to test? Plan? / 是否需要样件进行验证？计划？ | NA |
| Whether the catalyst information is confirmed? / 催化剂信息是否确认？（物料号、图纸、SPEC） | spec已执行（OCR模糊） |
...
| 字段 | 内容 |
|---|---|
| Interface & boundary (internal and customer) will be influenced? / 接口&边界（内部和客户）影响？ | [ ] yes / 是；[x] no / 否（OCR模糊：青去） |
| If impact customer, whether get confirm from customer side? / 请说明影响和是否得到客户的确认？ | （OCR模糊：青去） |
...
| 4.2 Product FMEA-relevant / DFMEA | [x] no / 否；[ ] yes / 是 | 有主项目已释放（OCR模糊：有主项日起释） |
...
| 4.6 TCD relevant | [x] NA（新零件放行，不涉及库存） |  |
...
| 字段 | 内容 |
|---|---|
| DFMA & Sample production OPL? | no change / 不变 |
| DFMA状态 | 催化剂提前放行无需DFMA（参考002） |
...
| 字段 | 内容 |
|---|---|
| PM alignment? (Influence on project Timeline? Cost?) / 是否同意对项目时间和成本的影响？ | （OCR模糊：34小s） |
| 需求样品订单数量 |  |
...
| 角色 | 姓名 | 签名 | 日期 |
|---|---|---|---|
| Design Team Leader / 设计团队负责人 | （OCR模糊：加） |  |  |
| System SM-ENG-CV1 / 系统 |  |  |  |
| Design / 设计工程师 | 卢松 |  | 2025.4.18 |
| Check / 校对工程师 | 杨拓 |  | 2025.04.18 |
| Catalyst / 催化剂 | 张江泉 |  |  |
| PUE / 采购 | 李岑 |  |  |
| PM / 项目经理 |  |  |  |
| TCR3 | 谢支福 |  |  |
