...
function DepartmentTaskCard({
  ...
  const [remark, setRemark] = useState(task.impact_remark || "")
  const [actionRequired, setActionRequired] = useState(task.action_required || "")
  ...
  const confirm = async () => {
    ...
    setError("")
    try {
      const next = await confirmPdEcrDepartmentTask(task.id, {
        impact_result: remark.trim() ? "impact" : "no_impact",
        impact_remark: remark,
  ...
  return (
    <div className="rounded border border-stone-200 bg-white p-3">
      ...
      <p className="mt-1 text-xs text-stone-500">
        {task.assignee_name || task.assignee_email || "未分配"}
      </p>
      {task.status === "confirmed" ? (
        <div className="mt-3 rounded bg-emerald-50 p-2 text-xs text-emerald-800">
          <p>{task.impact_remark || "已确认无补充备注"}</p>
          {task.confirmed_by_name && <p className="mt-1">确认人：{task.confirmed_by_name}</p>}
        </div>
      ) : (
        <div className="mt-3 space-y-2">
          <textarea
            ...
            placeholder="Impact remark / 影响说明"
          />
          <textarea
            ...
            placeholder="Action required / 后续措施"
          />
          {error && <p className="text-xs text-rose-600">{error}</p>}
          <Button type="button" size="sm" className="w-full bg-emerald-600 hover:bg-emerald-700" onClick={confirm} disabled={isSaving}>
            {isSaving ? "确认中..." : "部门确认"}
          </Button>
        </div>
      )}
...
function LeaderTaskCard({
  ...
  const review = async (decision: "approved" | "rejected" | "changes_requested") => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "签核失败")
    } finally {
      setIsSaving(false)
    }
  }
      ...
      {task.status === "approved" ? (
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button type="button" size="sm" className="bg-emerald-600 hover:bg-emerald-700" onClick={() => review("approved")} disabled={isSaving}>
              Approve
            </Button>
...
function WorkflowSignerPanel({ caseId, onComplete }: { caseId: string; onComplete?: () => void }) {
  const [workflow, setWorkflow] = useState<PdEcrWorkflowState | null>(null)
  const [selected, setSelected] = useState<Record<string, boolean>>({ design: true, quality: true })
  ...
  const submit = async () => {
    const selectedDepartments = WORKFLOW_DEPTS.filter((dept) => selected[dept.id]).map((dept) => dept.id)
    const assignees = Object.fromEntries(
      selectedDepartments.map((department) => [
        department,
        {
          assignee_email: emails[department] || "",
          assignee_name: emails[department] || "",
        },
      ]),
    )
    ...
    try {
      const next = await submitPdEcrWorkflow(caseId, {
        selected_departments: selectedDepartments,
        assignees,
      })
      setWorkflow(next)
      setStatusText("Department confirmation started")
    } catch (err) {
      setStatusText(err instanceof Error ? err.message : "Workflow submit failed")
    } finally {
    ...
    <div className="sticky top-4 space-y-4" style={{ maxHeight: "calc(100vh - 8rem)", overflowY: "auto" }}>
      <div className="rounded-lg border border-amber-300 bg-white shadow-sm">
        ...
        <div className="space-y-3 p-3">
          ...
          {!workflow?.department_tasks.length && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-stone-600">Affected departments</p>
              {WORKFLOW_DEPTS.map((dept) => (
                <label key={dept.id} className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 rounded border border-stone-100 p-2 text-xs">
                  <input
                    type="checkbox"
                  ...
                  <input
                    ...
                    onChange={(event) => setEmails((prev) => ({ ...prev, [dept.id]: event.target.value }))}
                    className="h-8 min-w-0 rounded border border-stone-200 px-2 outline-none focus:border-amber-400"
                    placeholder="assignee@email.com"
                    disabled={!selected[dept.id]}
                  />
                </label>
              ))}
              <Button type="button" className="w-full bg-stone-800 hover:bg-stone-700" onClick={submit} disabled={isSubmitting}>
                {isSubmitting ? "提交中..." : "提交部门确认"}
              </Button>
            </div>
          )}
        </div>
      ...
      {workflow?.department_tasks.length ? (
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Department confirmation</p>
          {workflow.department_tasks.map((task) => (
            <DepartmentTaskCard key={task.id} task={task} onRefresh={setWorkflow} />
          ))}
        </div>
      ) : null}
      ...
      {workflow?.leader_review_tasks.length ? (
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Leader review</p>
          {workflow.leader_review_tasks.map((task) => (
            <LeaderTaskCard key={task.id} task={task} onRefresh={setWorkflow} />
          ))}
        </div>
      ) : null}
      ...
      <div className={`grid gap-5 ${showApproval ? "xl:grid-cols-[1fr_17rem]" : ""}`}>
        ...
        {showApproval && (
          <div className="hidden xl:block">
            ...
            {caseId ? (
              <WorkflowSignerPanel caseId={caseId} onComplete={() => setViewMode("result")} />
            ) : (
