...
| Version | 13 |
| Effective date | 2024-08-08th |
| Design Change Request No. | 24_102 |
...
| If Yes, System impaction evaluation? / 如果是，系统影响评估？（如果是初次样件放行，空白处请说明样件目的） | 首批样件，无需验证 |
| Whether need sample to test? Plan? / 是否需要样件进行验证？计划？ | 首批样件，无需验证 |
...
| If Yes, How to evaluate? Plan? / 如果是，请说明影响和如何评估，计划？（例如：仿真分析等） | A样件无需FEA仿真 |
...
注：RZ量产改催化剂样件不涉及量产库存问题

| 库存地点 | 状态 | 库存数量 |
|---|---|---|
| RBCW RDC（外库）和RBCW仓库 | [ ] 无库存；[ ] 有库存 |  |
...
变更对采购成本影响：已勾选 decrease / 降低金额。手写备注为“样件费报价
...
| Does the change influence on purchasing cost? / 变更对采购成本的影响？ | [ ] increase / 增加，金额\_\_\_\_；[x] decrease / 降低，金额\_\_\_\_；[ ] no change / 不变 |
...
| 字段 | 内容 |
|---|---|
| Feedback from manufacturing engineer? / 生产制造装配是否影响，反馈？ | 无 |
...
| System SM-ENG-CV1  | （待确认） |  |  |
| TCR3 Department Manager / 部门经理 | 谢友福 |  | 2024.8.26 |
