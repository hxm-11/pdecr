...
const RESULT_MODULE_LABELS: Record<
  string,
  { title: string; subtitle: string }
> = {
  "validation-plan": {
    title: "3.1 QAC & Validation results",
    subtitle: "Validation Results",
  },
  "implementation-plan": {
    title: "3.2 Implementation results",
...
function completionState(
  label: ModuleCompletionState["label"],
  detail: string,
...
function changeDescriptionStatus(module: PdEcrDisplayModule) {
  ...
  const stored = safeParseJson(
    localStorage.getItem(
      `pd-ecr-change-description-draft:${recordId}:${module.id}`,
    ),
  );
  const data = module.data || {};
  const values = {
    summary: textValue(
      stored?.changeSummary,
  ...
  if (!filled && !hasGeneratedContent(module))
    return completionState("Empty", "No change description yet");
...
function impactAnalysisStatus(module: PdEcrDisplayModule) {
  const draft = safeParseJson(
    localStorage.getItem(`pd-ecr-impact-analysis-${module.id}`),
  );
  const impacts = Array.isArray(draft?.impacts)
    ? (draft.impacts as Array<Record<string, unknown>>)
    : [];
  if (!impacts.length) {
  ...
  if (unselected || yesWithoutMeasure) {
    ...
    if (yesWithoutMeasure)
      parts.push(`${yesWithoutMeasure} Yes missing measures`);
    return completionState("Needs input", parts.join(", "));
  }
  return completionState(
    "Complete",
    `${impacts.length} impact item${impacts.length > 1 ? "s" : ""} reviewed`,
  );
}

function validationPlanStatus(module: PdEcrDisplayModule) {
  const draft = safeParseJson(
    localStorage.getItem(`pd-ecr-validation-plan-${module.id}`),
  );
  ...
  const impactRows = Array.isArray(impactDraft?.impacts)
  ...
  const selectedMissing = selected.filter(
    (row) => !textValue(row.respPerson) || !textValue(row.finishDate),
  ).length;
  ...
  if (!selected.length && !customMeasureIndexes.length) {
    return completionState(
      "Needs input",
      "Select at least one validation item",
    );
  }
  if (selectedMissing || customMissing) {
    ...
    return completionState(
      "Needs input",
      `${total} validation item${total > 1 ? "s" : ""} missing owner/date`,
    );
  }
  return completionState(
    "Complete",
    `${selected.length + customMeasureIndexes.length} validation item${selected.length + customMeasureIndexes.length > 1 ? "s" : ""} planned`,
  );
}

function implementationPlanStatus(module: PdEcrDisplayModule) {
  const draft = safeParseJson(
    localStorage.getItem(`pd-ecr-implementation-${module.id}`),
  );
  const rows = Array.isArray(draft?.checklistRows)
    ? (draft.checklistRows as Array<Record<string, unknown>>)
    : [];
  if (!rows.length) {
    return hasGeneratedContent(module)
  ...
  const activeRows = rows.filter(
    (row) => String(row.yn || "").toUpperCase() === "Y",
  );
  const missing = activeRows.filter(
    (row) =>
      !textValue(row.description) ||
      !textValue(row.responsible) ||
      !textValue(row.dueDate),
  ).length;
  ...
  if (missing) {
    return completionState(
      "Needs input",
      `${missing} Y item${missing > 1 ? "s" : ""} missing owner/date`,
    );
  }
  return completionState(
    "Complete",
    `${activeRows.length} implementation item${activeRows.length > 1 ? "s" : ""} ready`,
  );
}

export function getModuleCompletionState(
    ...
    <div className="overflow-hidden rounded-xl border border-stone-200/60 bg-white shadow-sm card-hover">
      ...
      >
        ...
        <div className="hidden shrink-0 flex-col items-end gap-1 md:flex">
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ${state.className}`}
          >
            {state.icon}
            {state.label}
          </span>
          ...
          {module.warnings?.length ? (
            <span className="text-[11px] font-medium text-amber-700">
              {module.warnings.length} warning
              {module.warnings.length > 1 ? "s" : ""}
            </span>
          ) : null}
        </div>
...
function GroupedPlanningItem({
  impactModule,
  validationModule,
  isExpanded,
  onToggle,
}: {
  impactModule?: PdEcrDisplayModule;
  validationModule?: PdEcrDisplayModule;
  isExpanded: boolean;
  onToggle: () => void;
}) {
  const [statusRevision, setStatusRevision] = useState(0);
  const modules = [impactModule, validationModule].filter(
    Boolean,
  ) as PdEcrDisplayModule[];
  const hasContent = modules.some(hasGeneratedContent);
  const states = modules.map((module) => ({
    module,
    state: getModuleCompletionState(module),
    label: MODULE_LABELS[module.id] || {
      title: module.title,
      subtitle: module.subtitle,
    },
  }));
  const completeCount = states.filter(
    (item) => item.state.label === "Complete",
  ).length;
          ...
          {states.map(({ module, state, label }) => (
            ...
            >
              <div className="mb-4 flex flex-col justify-between gap-2 border-b border-stone-200 pb-3 md:flex-row md:items-center">
                ...
                <span
                  className={`inline-flex w-fit items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold ${state.className}`}
                >
                  {state.icon}
                  {state.label}
                </span>
              ...
              {renderModuleBody(module, true, "plan")}
            </section>
          ))}
        </div>
      )}
...
export function PdEcrResultModuleAccordion({
  modules,
}: {
    ...
    <div className="space-y-3">
      {resultModules.map((module) => (
        <AccordionItem
          ...
          module={module}
          label={
            RESULT_MODULE_LABELS[module.id] || {
              title: module.title,
              subtitle: module.subtitle,
            }
          }
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed) && parsed.length)
        return parsed as ValResultDetail[];
    }
  } catch {
    /* ignore */
  }
...
function loadImplementationResults(moduleId: string) {
  ...
  return {
    checklistRows: [] as ImplResultRow[],
...
export function ResultView({ modules }: { modules: PdEcrDisplayModule[] }) {
  ...
  const valResultDetails = valModule
    ? loadValidationResultDetails(valModule.id)
    : [];
  const implData = implModule
    ? loadImplementationResults(implModule.id)
    : {
        checklistRows: [] as ImplResultRow[],
        developmentConfirmation: "",
        implementationDate: "",
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {valRows.length ? (
                valRows.map((row, i) => {
                  const detail = valResultDetails.find(
                    (d) => d.item === row.label,
                  );
                  return (
                    <tr
...
export function PdEcrModuleAccordion({
  ...
  const impactModule = modules.find(
    (module) => module.id === "impact-analysis",
  );
  const validationModule = modules.find(
    (module) => module.id === "validation-plan",
  );
  const implementationModule = modules.find(
    (module) => module.id === "implementation-plan",
  );
  const hasPlanModules = Boolean(
    impactModule || validationModule || implementationModule,
  );
  ...
  return (
    <div className="relative">
      <div
        className={`grid gap-5 ${showApproval && Boolean(caseId) ? "xl:grid-cols-[1fr_17rem]" : ""}`}
      >
        {/* ── LEFT: Accordion modules ── */}
        <div className="space-y-3 min-w-0 min-h-0">
          {(impactModule || validationModule) && (
            <GroupedPlanningItem
              impactModule={impactModule}
              validationModule={validationModule}
              isExpanded={expandedIds.includes("planning")}
              onToggle={() => handleToggle("planning")}
            />
          )}
          {implementationModule && (
            <AccordionItem
              key={implementationModule.id}
              module={implementationModule}
              label={
                MODULE_LABELS[implementationModule.id] || {
                  title: implementationModule.title,
                  subtitle: implementationModule.subtitle,
                }
              }
              isExpanded={expandedIds.includes(implementationModule.id)}
              onToggle={() => handleToggle(implementationModule.id)}
            />
          )}
        </div>
